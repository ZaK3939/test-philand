...
export function shouldBehaveCreateObject(): void {
  it("should createObject", async function () {
    ...
    expect(await this.paidObject.connect(this.signers.alice).getCreator(2)).to.equal(this.signers.bob.address);
...
export function shouldBehaveInitObject(): void {
  it("should initObject", async function () {
    ...
    expect(await this.paidObject.connect(this.signers.alice).getMaxClaimed(2)).to.equal(300);
    expect(await this.paidObject.connect(this.signers.alice).uri(2)).to.equal(
      "https://www.arweave.net/jRkF9OhcOzglECJnKtbS1PsICoBlCH6HDuCW8EVePNk",
    );
    expect(await this.paidObject.connect(this.signers.alice).getCreator(2)).to.equal(this.signers.carol.address);
    const size = await this.paidObject.connect(this.signers.admin).getSize(2);
...
export function shouldBehaveBuyObject(): void {
  it("should buy object with eth", async function () {
    ...
    const beforeNFTbalance = await this.paidObject.connect(this.signers.alice).balanceOf(this.signers.alice.address, 2);
    expect(beforeNFTbalance).to.equal(0);
    const ethToSend = ethers.utils.parseEther("0.05");
    await this.paidObject.connect(this.signers.alice).buyObject(2, { value: ethToSend });
    const afterNFTbalance = await this.paidObject.connect(this.signers.alice).balanceOf(this.signers.alice.address, 2);
...
export function CantBuyObjectWithNotEnoughETH(): void {
  it("cant buy object with not enough eth", async function () {
    const price = await this.paidObject.connect(this.signers.admin).getTokenPrice(2);
    expect(price).to.equal(ethers.utils.parseEther("0.05"));
    const beforebalance = await this.paidObject.connect(this.signers.bob).balanceOf(this.signers.bob.address, 2);
    expect(beforebalance).to.equal(0);
    const ethToSend = ethers.utils.parseEther("0.01");
    await expect(this.paidObject.connect(this.signers.bob).buyObject(2, { value: ethToSend })).to.be.reverted;
  });
}
